...
def gen():
    ...
    if not region or region.upper() not in REGION_URLS:
...
def load_proxies():
    ...
    try:
        ...
        print(f"{red}Error: proxy.txt file not found!{Style.RESET_ALL}")
        return []
    except Exception as e:
        print(f"{red}Error reading proxy.txt: {str(e)}{Style.RESET_ALL}")
        return []
...
def create_acc(region):
    ...
    try:
        ...
        print(f"{red}Error in create_acc: {str(e)}{Style.RESET_ALL}")
        return create_acc(region)  # Retry with a new proxy
...
def GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region):
    ...
    while attempt < max_retries:
        try:
            ...
            attempt += 1
            proxy = get_random_proxy(proxies)  # Try a new proxy on retry
            time.sleep(2)
    return None
    ...
    for i in range(quantity):
        try:
            ...
            if r.get('status_code') == 200:
                ...
                accounts.append({"error": f"Failed to create account: Status code {r.get('status_code')}"})
        except Exception as e:
